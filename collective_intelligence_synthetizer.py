...
class CollectiveIntelligenceSynthesis(dspy.Signature):
    ...
    previous_summary: Optional[str] = dspy.InputField(
        desc="accumulated synthesis summary from previously processed transcript chunks"
    )
    current_transcripts: List[str] = dspy.InputField(
        desc="current chunk of congressional session transcripts to analyze"
    )

    synthesis_summary: str = dspy.OutputField(
        desc="enhanced synthesis summary incorporating both previous knowledge and new transcript analysis"
    )
...
class CollectiveIntelligenceSynthetizer(dspy.Module):
    ...
    def _chunk_transcripts(self, transcripts: List[str]) -> List[List[str]]:
        ...
        return [transcripts[i : i + self.chunk_size] for i in range(0, len(transcripts), self.chunk_size)]
    ...
    def forward(self, original_query: str, congress_transcripts: List[str]) -> dspy.Prediction:
        ...
        return dspy.Prediction(collective_answer=accumulated_summary, total_chunks_processed=len(chunks))

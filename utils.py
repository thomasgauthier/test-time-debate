    ...
    **kwargs,
    ...
    while True:
        try:
            ...
            console.print(
                f"[bold red]⚠️  {component.__class__.__name__} failed (Attempt {attempt + 1}). Error: {str(e)}. Retrying in {wait_time}s...[/]"
            )
...
def extract_quotes_from_markdown(text: str) -> list[str]:
    ...
    for tag in strong_tags:
        ...
        if (
            (tag_text.startswith('"') or tag_text.startswith("“"))
            and (tag_text.endswith('"') or tag_text.endswith("”"))
        ) and len(tag_text) > 1:
...
def verify_and_wrap_quotes(response_text: str, source_document: str) -> str:
    ...
    for quote in quotes:
        ...
        if quote in source_document:
            ...
            response_text = response_text.replace(f'**"{quote}"**', f'<v_quote>"{quote}"</v_quote>')

            response_text = response_text.replace(f'**"{quote}"**', f'<v_quote>"{quote}"</v_quote>')
        else:
            # Replace with unverified quote wrapper
            response_text = response_text.replace(f'**"{quote}"**', f'<u_quote>"{quote}"</u_quote>')

...
class SwarmIntelligenceReduction(dspy.Signature):
    ...
    previous_insights: List[str] = dspy.InputField(
        desc="insights extracted from previously processed conversation chunks"
    )
    current_conversations: List[str] = dspy.InputField(
        desc="current chunk of congressional session transcripts to analyze"
    )

    updated_insights: List[str] = dspy.OutputField(
        desc="refined and expanded insights combining previous knowledge with new findings"
    )
    new_discoveries: List[str] = dspy.OutputField(
        desc="novel insights discovered in the current chunk that weren't present before"
    )
...
class SwarmIntelligenceReducer(dspy.Module):
    ...
    def _chunk_conversations(self, conversations: List[str]) -> List[List[str]]:
        ...
        return [conversations[i : i + self.chunk_size] for i in range(0, len(conversations), self.chunk_size)]
    ...
    def forward(self, conversations: List[str]) -> dspy.Prediction:
        ...
        for chunk_idx, chunk in enumerate(chunks):
            result = self.reducer(previous_insights=accumulated_insights, current_conversations=chunk)
